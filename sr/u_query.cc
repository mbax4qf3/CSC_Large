...
int main(int argc, char** argv) {
    ...
    if (qtype == "i") {
        ...
        std::string upd;
        upd = "info/" + file_index + "singleUPD.txt";
        std::ofstream outfileUPD;
        outfileUPD.open(upd.c_str());
        ...
        for (auto inc_e : inc_edge) {
            ...
            const auto beg_s = std::chrono::steady_clock::now();
            cnt_list.push_back(uspc.IncEdge(inc_e.first,inc_e.second,mini_fg));
            const auto end_s = std::chrono::steady_clock::now();
            const auto dif_s = end_s - beg_s;

            outfileUPD << std::chrono::duration<double, std::micro>(dif_s).count() << std::endl;
        }
